...
@Component({
  selector: 'spr-confirm-modal-initializer',
  template: ` <spr-button (click)="showConfirmModal()">Show Confirm Modal</spr-button> `,
  imports: [SprButtonComponent],
})
class ConfirmModalInitializerComponent {
  title = input('Confirm Action title');
  ...
  showConfirmModal(): void {
    ...
    this.modalService
      .open<ConfirmModalData, boolean>(SprConfirmModalComponent, {
        modalData,
...
Use this component when you need user confirmation before performing critical actions like deletions or irreversible changes.
  ...
  decorators: [
    moduleMetadata({
      imports: [SprConfirmModalComponent],
