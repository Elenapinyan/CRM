...
const meta: Meta<SprChipsComponent & CustomArgs> = {
  title: 'shared components/Chips',
  component: SprChipsComponent,
  parameters: {
    docs: {
      description: {
        component: `
...
Some statuses may intentionally share a similar or even the same color (e.g., several "in progress" states using Blue). To keep them clearly distinguishable, it's recommended to pair them with different icons that reinforce meaning (e.g., clock for "Scheduled", spinner/loader for "Processing", pause for "On hold").
  ...
  argTypes: {
    variant: {
      options: ['gray', 'olive', 'green', 'purple', 'blue', 'primary', 'gold', 'red', 'cyan', 'orange', 'magenta', 'yellow'],
      control: {
        type: 'select',
      },
      ...
      table: {
        type: {
          summary: 'gray | olive  | green | purple | blue | primary | gold | red | cyan | orange |magenta | yellow',
        },
        defaultValue: {
          summary: 'primary',
        },
...
export const Chips: StoryObj<SprChipsComponent & CustomArgs> = {
  args: {
    variant: 'gray',
    size: 'md',
    chipsStyle: 'filled',
  ...
  render: (args: SprChipsComponent & CustomArgs) => {
    return {
      props: { ...args },
      template: `
        <spr-chips
          ...
          <i
            *ngIf="leftIcon"
            class="bo-icon-general-check-shield"
          ></i>
        ...
        <spr-chips
          ...
          <i class="bo-icon-general-check-shield"></i>
