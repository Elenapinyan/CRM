...
@Component({
  selector: 'spr-transfer-list',
  templateUrl: './spr-transfer-list.component.html',
  styleUrls: ['./spr-transfer-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [CommonModule, ReactiveFormsModule, SprInputComponent, SprButtonComponent, SprInnerHeaderComponent, TransferListItemComponent],
})
export class SprTransferListComponent {
  ...
  protected customTemplates = contentChildren(SprTransferListTemplateDirective);
  ...
  protected readonly customTemplateDict = computed(() => {
    ...
    for (const dir of customTemplates) {
      dictionary[dir.sprTransferListTemplate()] = dir.template;
    }
  ...
  private createSortedUnits(items: TransferListItem[]): TransferListItem[] {
    ...
    for (const item of items) {
      ...
      const grouped = collection[key] ?? { key, items: [] };

      grouped.items.push(item);

      collection[key] = grouped;
    }
