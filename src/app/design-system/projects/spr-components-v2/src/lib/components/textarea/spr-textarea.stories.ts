...
const meta: Meta<DsTextareaComponent & { formControl: FormControl }> = {
  title: 'shared components/Textarea',
  component: DsTextareaComponent,
  decorators: [
    moduleMetadata({
      imports: [ReactiveFormsModule, DsButton],
    }),
  ],
  parameters: {
    docs: {
...
export const Textarea: StoryObj<DsTextareaComponent & { formControl: FormControl }> = {
  args: {
    ...
    return {
      ...
      template: `
       ...
       ></ds-textarea>
        ...
        <ds-button style="padding: 16px 0" variant="flat" (click)="area.focus()">Manual focus</ds-button>
      `,
    };
  },
};
