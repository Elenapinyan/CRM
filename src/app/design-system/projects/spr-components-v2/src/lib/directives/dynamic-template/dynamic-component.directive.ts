...
@Directive({
  selector: '[dsDynamicComponent]',
})
export class DsDynamicComponentDirective<T = unknown> extends DsDynamicBaseDirective<T> {
  @Input() set dsDynamicComponentPosition(value: DynamicPosition) {
  ...
  @Input() set dsDynamicComponentProjection(value: DynamicProjection) {
    if (this.componentProjection === value) {
      return;
    }

    this.componentProjection = value;

    this.dElement?.updateProjection(value);
  }

  @Input() set dsDynamicComponentCustomContainer(value: ElementRef<HTMLElement> | HTMLElement | undefined) {
    if (!this.dElement) {
      return;
    }
  ...
  set dsDynamicComponent(value: Type<T> | undefined) {
    ...
    if (this.dElement && this.dsDynamicComponentClass) {
      this.dElement.dynamicClass = this.dsDynamicComponentClass;
    }
  ...
  private componentProjection?: DynamicProjection;
